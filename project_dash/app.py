...
df_percentage = pd.read_csv(
    f'../project_dash/storage/data_percentage_amount/percentage_weekly.csv')
df_amount = pd.read_csv(
    f'../project_dash/storage/data_percentage_amount/amount_weekly.csv')

# Data referente ao dia anterior ao dia da extração dos dados
date_final = date.today() - timedelta(1)
# Data "inicial" da semana que vai até a data presente no date_final
date_initial = date_final - timedelta(6)

exist = False
colunas = df_percentage.columns.values
colunas = np.delete(colunas, 0)

for col in colunas:
    if col.__contains__('.') == False:
        col = datetime.strptime(col, '%Y-%m-%d').date()
        if date_final == col:
            exist = True

if exist == False:
    coleta_dados.main(names, date_initial, date_final)

app = dash.Dash(__name__, external_stylesheets=[dbc.themes.BOOTSTRAP])

# Coleta e ajustes nos dataframes
df_aux_percentage = df_percentage.reset_index(drop=True).set_index('Nome')
df_aux_percentage = df_aux_percentage.T

df_aux_amount = df_amount.reset_index(drop=True).set_index('Nome')
df_aux_amount = df_aux_amount.T
